...
class ReviewCache(GObject.Object):
    ...
    def _update_reviews_thread(self):
        ...
        self.proc = multiprocessing.Process(target=_update_cache_process, args=(success, current_size))
        self.proc.start()
...
def _update_cache_process(success, current_size):
    new_reviews = {}
    ...
    try:
        ...
        if r.status_code == 200:
            if int(r.headers.get("content-length")) != current_size.value:
                ...
                _save_cache(new_reviews, r.headers.get("content-length"))
